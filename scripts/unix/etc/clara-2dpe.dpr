...
# Check to see if port 7881 is in use. if so exit.
exec 6<>/dev/tcp/127.0.0.1/7881 || dpe_port=up
if [ -z ${dpe_port+x} ]; then
echo "Found active running Clara on the local host. Exiting..."
exit
fi

# Check to see if FE DPE is running on the local host.
# If not fe_port will be set to up, otherwise the variable fe_port will not be defined.
exec 6<>/dev/tcp/127.0.0.1/7771 || fe_port=up
#clear
...
if [ -z ${fe_port+x} ]; then
echo " Log file           = Clara distribution/log/$HOST-jdpe.log"
echo "------------------------------------------"
echo
j_dpe --port 7881 --fe-host $FE_HOST --session $1 2>&1 | tee "$LOG_FILE-jdpe.log"
else

echo " Log file           = Clara distribution/log/$HOST-jfe.log"
echo " Note               = Running as local Front-End"
echo "------------------------------------------"
echo
j_dpe --host $FE_HOST --session $1 2>&1 | tee "$LOG_FILE-jfe.log"
fi
